...
As we extend our discussion of thermodynamic concepts toward the objective of predicting spontaneity, consider now an isolated system consisting of two flasks connected with a closed valve. Initially there is an ideal gas on the left and a vacuum on the right ({'Figure_16_2_Gas Figure 16.2.Gas}). When the valve is opened, the gas spontaneously expands to fill both flasks. Recalling the definition of pressure-volume work from the chapter on thermochemistry, note that no work has been done because the pressure in a vacuum is zero.
