...
model module Foundations
  ...
  model module Epidemiology
    ...
    model *Superspreading
      head = A superspreading event (SSEV) is an event in which an infectious disease is spread much more than usual, while an unusually contagious organism infected with a disease is known as a superspreader.

      / 80/20 Rule
      model *8020Rule
        head = Some cases of superspreading conform to the 80/20 rule, where approximately 20% of infected individuals are responsible for 80% of transmissions, although superspreading can still be said to occur when superspreaders account for a higher or lower percentage of transmissions.

    # Overdispersion - https://en.wikipedia.org/wiki/Overdispersion
    model *Overdispersion
      head = Overdispersion is the presence of greater variability (statistical dispersion) in a data set than would be expected based on a given statistical model. Overdispersion is a very common feature in applied data analysis because in practice, populations are frequently heterogeneous (non-uniform) contrary to the assumptions implicit within widely used simple parametric models.
      >
